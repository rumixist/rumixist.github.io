    ...
    if (timestampElement) {
        if (timestamp) {
            try {
                ...
                timestampElement.textContent = `Hesaba katılan son deprem: ${formattedDate}`;
            } catch (e) {
                console.error("Tarih formatlama hatası:", e);
                 timestampElement.textContent = `Hesaba katılan son deprem: Bilinmiyor (Tarih hatası)`;
            }
        } else {
            timestampElement.textContent = `Hesaba katılan son deprem: Veri alınamadı`;
        }
    }
};
